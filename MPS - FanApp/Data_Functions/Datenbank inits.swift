...
func createTables(){
    let querrysToCreate =
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS `Fest_has_Band` ( `Fest_name` VARCHAR(255) NOT NULL, `Band_Name` VARCHAR(255) NOT NULL, PRIMARY KEY (`Fest_name`, `Band_Name`), INDEX `fk_Fest_has_Band_Band1_idx` (`Band_Name` ASC) VISIBLE, INDEX `fk_Fest_has_Band_Fest_idx` (`Fest_name` ASC) VISIBLE, CONSTRAINT `fk_Fest_has_Band_Fest` FOREIGN KEY (`Fest_name`) REFERENCES `Fest` (`name`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_Fest_has_Band_Band1` FOREIGN KEY (`Band_Name`) REFERENCES `Band` (`Name`) ON DELETE NO ACTION ON UPDATE NO ACTION)
        """,
        """
        CREATE TABLE IF NOT EXISTS `Fest_has_Marktstand` ( `Fest_name` VARCHAR(255) NOT NULL, `Marktstand_name` VARCHAR(255) NOT NULL, PRIMARY KEY (`Fest_name`, `Marktstand_name`), INDEX `fk_Fest_has_Marktstand_Marktstand1_idx` (`Marktstand_name` ASC) VISIBLE, INDEX `fk_Fest_has_Marktstand_Fest1_idx` (`Fest_name` ASC) VISIBLE, CONSTRAINT `fk_Fest_has_Marktstand_Fest1` FOREIGN KEY (`Fest_name`) REFERENCES `Fest` (`name`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_Fest_has_Marktstand_Marktstand1` FOREIGN KEY (`Marktstand_name`) REFERENCES `Marktstand` (`name`) ON DELETE NO ACTION ON UPDATE NO ACTION)
        """,
        """
        CREATE TABLE IF NOT EXISTS `Fest_has_Lager` (`Fest_name` VARCHAR(255) NOT NULL,`Lager_Name` VARCHAR(255) NOT NULL,PRIMARY KEY (`Fest_name`, `Lager_Name`),INDEX `fk_Fest_has_Lager_Lager1_idx` (`Lager_Name` ASC) VISIBLE,INDEX `fk_Fest_has_Lager_Fest1_idx` (`Fest_name` ASC) VISIBLE,CONSTRAINT `fk_Fest_has_Lager_Fest1` FOREIGN KEY (`Fest_name`) REFERENCES `Fest` (`name`) ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT `fk_Fest_has_Lager_Lager1` FOREIGN KEY (`Lager_Name`) REFERENCES `Lager` (`Name`) ON DELETE NO ACTION ON UPDATE NO ACTION)
        """]
