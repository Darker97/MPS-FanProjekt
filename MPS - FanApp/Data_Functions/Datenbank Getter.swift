...
func Get_Fest_all(db:OpaquePointer) -> [String]{
    ...
    return execute_withReturn(Query: query, ErgebnisZeilen: 5)
}
func Get_Lager_all(db:OpaquePointer) -> [String]{
    let query = "SELECT * FROM Lager"
    return execute_withReturn(Query: query, ErgebnisZeilen: 4)
}
...
func Get_Fest(db:OpaquePointer, Fest: String) -> [String]{
    let query = "SELECT * FROM Fest WHERE Name = \"" + Fest + "\""
    return execute_withReturn(Query: query, ErgebnisZeilen: 5)
}
...
func Get_FromFest_Lager(db:OpaquePointer, Fest:String) -> [String]{
    let query = "SELECT * FROM Lager WHERE Fest_name =\"" + Fest + "\""
    return execute_withReturn(Query: query, ErgebnisZeilen: 4)
}
func Get_FromFest_Marktstand(db:OpaquePointer, Fest:String) -> [String]{
    let query = "SELECT * FROM Marktstand WHERE Fest_name =\"" + Fest + "\""
    return execute_withReturn(Query: query, ErgebnisZeilen: 4)
}
func Get_FromFest_Band(db:OpaquePointer, Fest:String) -> [String]{
    let query = "SELECT * FROM Band WHERE Fest_name =\"" + Fest + "\""
    return execute_withReturn(Query: query, ErgebnisZeilen: 5)
}
